...
class Pager {
  ...
  initHtml () {
    let pager = this.domRefs.pager = dom.create(`<nav class="pager"></nav>`)
  ...
  bindEvents () {
    dom.on(this.options.element, 'click', `[data-role="pageNumbers"] > li`, (e, el) => {
  ...
  goToPage (page) {
    ...
    //  只能用detail
    // this.options.element.dispatchEvent(new CustomEvent('pageChange', {haha: {page}}))
    this.rerender()
  }
  rerender () {
    this._checkButtons()
  ...
  _createNumbers () {
    ...
    for (var i = start; i <= end; i ++) {
      let li = dom.create(`<li data-page="${i}">${i}</li>`)
      if (i === currentPage) {
